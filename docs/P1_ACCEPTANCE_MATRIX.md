...
| P1-REG-02 | P0 frontend kabulü regresyonsuzdur | 35/35 component testi, lint, typecheck ve production build | ✅ Kanıtlandı |
...
| Browser | Production Compose Desktop Chrome ve Pixel 7 | P0 ve P1 toplam 6/6 geçti; [masaüstü](screenshots/pr6-p1-event-search-desktop.png) ve [mobil](screenshots/pr6-p1-event-search-mobile.png) görsel kanıtı eklendi |
| Graceful shutdown | Linux container `docker stop` probe ve POSIX alt süreç integration testi | HTTP `200`, lifespan stopped marker; Docker exit `0`, Uvicorn 0.52 yeniden yükseltilmiş `SIGTERM` davranışı doğrulandı |
