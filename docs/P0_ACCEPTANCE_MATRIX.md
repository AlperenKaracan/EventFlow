...
| Frontend component test   | `pnpm test`                                                                                    | 11 dosyada 28/28 test geçti; cursor, state, auth-race, timezone, koyu tema ve kategori renkleri dahil    |
| Frontend production build | `pnpm build` ve Docker multi-stage build                                                       | Geçti; ana giriş `315.17 kB` (`100.70 kB` gzip), form lazy chunk'ı `268.49 kB` (`82.91 kB` gzip)         |
| Görsel/UX regresyonu      | Masaüstü + mobil ekran görüntüsü, Türkçe tarih seçici ve kayıt geri bildirimi                  | Koyu tema, kategori renkleri, yeni form, konum/saat dilimi seçimleri ve kayıt yönlendirmesi doğrulandı   |
