...
| Frontend production build | `pnpm build` ve Docker multi-stage build                                                       | Geçti; ana giriş `315.33 kB` (`100.67 kB` gzip), form lazy chunk'ı `268.50 kB` (`82.90 kB` gzip)         |
| Görsel/UX regresyonu      | Keşif, auth, organizer ve form için masaüstü + mobil ekran görüntüsü                           | Düz koyu yüzeyler, kategori renkleri, sade auth, bilgi hiyerarşisi ve responsive form doğrulandı         |
